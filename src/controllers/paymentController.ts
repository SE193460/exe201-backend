...
export async function generateQR(req: Request, res: Response) {
  ...
  try {
    ...
    // Create a QR_GENERATED transaction with auto-generated payment code
    ...
    const qrUrl = `https://img.vietqr.io/image/MOMO-${MOMO_PHONE}-compact2.png?amount=${amount}&addInfo=${encodeURIComponent(paymentCode)}&accountName=${encodeURIComponent(MOMO_NAME)}`;
...
export async function adminConfirmPayment(req: Request, res: Response) {
  ...
  try {
    ...
    // Handle contact view credit packages (no listing_id = contact view purchase)
    if (!txn.listing_id) {
      const views = txn.amount === 5000 ? 5 : 20;
      await pool.query(
